...
def split_compound(word, word_list):
    ...
    for i in range(3, len(word) - 2):
        ...
        if prefix_lower in word_list:
            # 1. Fall: Direkter Split (z.B. Arbeit | amt)
            for rest in split_compound(suffix, word_list):
                yield [prefix] + rest

            # 2. Fall: Fugenelement 's' (z.B. Arbeit | s | amt)
            # Suffix muss mit 's' beginnen und danach noch min. 3 Zeichen haben, also muss suffix in diesem fall größer 3 sein
            if suffix.lower().startswith('s') and len(suffix) > 3:
...
def main():
    ...
    for original_word in nomen_counts:
        ...
        # Generiere alle Zerlegungen (Set verhindert Duplikate durch das Fugen-s)
        seen_splits = set()

        for parts in split_compound(original_word, word_lookup):
            split_tuple = tuple(parts)
            if split_tuple not in seen_splits:
                # Frequenzen der Teile holen
                freqs = [word_lookup[p.lower()] for p in parts]
                score = get_geometric_mean(freqs)
                all_splits.append((score, parts))
                seen_splits.add(split_tuple)
